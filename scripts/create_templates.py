#!/usr/bin/env python3
# -*- coding: utf-8 -*-
"""Korean Translation Template Generator"""
...
INTELLIJ_SRC = WORK_DIR / "extracted_bundles" / "intellij" / "messages"
ANDROID_SRC = WORK_DIR / "extracted_bundles" / "android" / "messages"
OUTPUT_DIR = WORK_DIR / "src" / "main" / "resources" / "messages"

print("=" * 60)
print("Korean Translation Template Generator")
print("=" * 60)
print()

def create_korean_template(source_file, output_file):
    """원본 properties 파일로부터 한국어 템플릿 생성"""
    try:
        ...
        # 헤더 추가
        header = f"""# ============================================================================
# {output_file.name} - Korean Translation
# Generated from {source_file.name}
# ============================================================================
#
# 번역 가이드:
# 1. 키 값은 절대 변경하지 마세요
# 2. = 뒤의 영문 메시지만 한국어로 번역하세요
# 3. {{0}}, {{1}} 같은 플레이스홀더는 그대로 유지하세요
# 4. &F, &E 같은 단축키 표시는 한국어 철자에 맞게 변경하세요
#    예: File(&F) → 파일(&F)
# 5. ... (말줄임표)는 유지하세요
#
# ============================================================================
...
"""
        ...
        print(f"    → 생성됨 ({output_file.relative_to(WORK_DIR)})")
        return True
...
for bundle_name in PRIORITY_BUNDLES:
    ...
    # IntelliJ 번들
    intellij_file = INTELLIJ_SRC / f"{bundle_name}.properties"
    if intellij_file.exists():
        output = OUTPUT_DIR / f"{bundle_name}_ko.properties"
        if create_korean_template(intellij_file, output):
            created_count += 1
        continue

    # Android 번들
    android_file = ANDROID_SRC / f"{bundle_name}.properties"
    if android_file.exists():
        output = OUTPUT_DIR / f"{bundle_name}_ko.properties"
        if create_korean_template(android_file, output):
            created_count += 1
        continue

    print(f"    ⚠ 원본 파일을 찾을 수 없음")

print(f"\n생성된 템플릿: {created_count} 개")

# 번역 가이드 문서 생성
print()
print("[2/2] 번역 가이드 생성 중...")

guide_content = """# Android Studio 한국어 언어팩 번역 가이드
...
guide_file = WORK_DIR / "TRANSLATION_GUIDE.md"
with open(guide_file, 'w', encoding='utf-8') as f:
    f.write(guide_content)

print(f"  번역 가이드 생성됨: {guide_file.name}")
...
print("=" * 60)
print(f"\n다음 단계:")
print(f"1. {guide_file.name} 파일을 읽어보세요")
print(f"2. src/main/resources/messages/ 에서 우선순위 번들부터 번역하세요")
print(f"3. 번역 후 ./gradlew buildPlugin 으로 빌드하세요")
print(f"\n우선순위 번들:")
for i, bundle in enumerate(PRIORITY_BUNDLES[:10], 1):
    print(f"  {i:2d}. {bundle}_ko.properties")
