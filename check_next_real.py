...
with open('missing_translations/missing_keys.json', 'r', encoding='utf-8') as f:
    data = json.load(f)
...
for file_entry in data.get('new_files', []):
    ...
    korean_file_entry = next((f for f in korean_data.get('new_files', []) if f['filename'] == filename), {})
    korean_keys = korean_file_entry.get('keys', {})
    
    missing_keys = {k: v for k, v in keys.items() if k not in korean_keys}
    
    if missing_keys:
        print(f"Next file: {filename} (Missing {len(missing_keys)}/{len(keys)} keys)")
        print(json.dumps(dict(list(missing_keys.items())[:50]), indent=2, ensure_ascii=False))
        break
else:
    print("All new files caught up! Checking existing files...")
